...
func configureSnapshotSlotConnection(
	ctx context.Context,
	conn snapshotSlotConnection,
) error {
	settings := []struct {
		query string
		name  string
		...
		// this otherwise-idle connection stays open. A source-level
		// wal_sender_timeout must not invalidate a long initial copy or resync.
